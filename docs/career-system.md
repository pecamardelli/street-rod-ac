...
| Event | Race | Filter | Min Rep | Reward |
|-------|------|--------|---------|--------|
| Ford Fanatics | Drag | BrandFilter("Ford") | 0 | $500 + 5 rep |
| Budget Brawl | Drag | ValueFilter(0, 5000) | 0 | $1000 + 3 rep |
| Muscle Madness | Drag | USA + 300+ HP | 25 | $750 + 10 rep + the best camshaft that fits |
| Classic Showdown | Road | Pre-1970 | 15 | $750 + 10 rep |
| Fifties Fever | Road | 1950s | 30 | $1000 + 8 rep |
...
- **A pink-slip event only draws a racer of the pool** (`EventOpponentService`): the car has to go into somebody's
  garage. With none free, the player is told nobody is putting a pink slip up today.
- **The prize camshaft** (`rare_camshaft`) is the dearest camshaft from the parts catalog that goes where the
  winning engine's camshaft sits and is dearer than its own, one for each such camshaft, put on the player's
  shelf (`PrizeParts.BestCamshaft`). When none fits, $500 (scaled by the part prices) is paid instead.
- **Reputation is read from the career's `ReputationReached` counter**, which `CareerState.SyncStanding` keeps at
  the player's reputation (with `CarsOwned`): at a new game, every morning before the events are generated,
  after a purchase, a sale and a race, and when the career screen opens.
...
Events are generated by `RaceEventService.GenerateEvents()`, at a new game and every morning by
`EventGenerationTask`.
...
4. **Check for victory**: Call `VictoryConditionService.ClaimVictory(gameState)`: the picked path, once reached, wins the game
...
- **Only the path the player picked wins** (`CareerState.ActiveVictoryType`, set with PURSUE on the career screen).
  With none picked, the first path reached wins. A path reached on the way waits: picking it later wins the game
  there and then.
- **`VictoryConditionService.ClaimVictory`** marks the game won (`HasWonGame`, `WinningVictoryType`), once. It is
  called after every race (`CareerProgressService`) and when a path is picked.
- **The victory screen** (`Dialogs/Victory`) shows the path and the career sheet (`VictoryCard`). The player keeps
  racing on the same save, which stays marked as won, or goes to the main menu. After the win no other path can be
  pursued.
