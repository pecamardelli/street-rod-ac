...
public static class AcdFile
{
    ...
    public static List<(string Name, byte[] Content)> Read(string acdPath)
    {
        ...
        while (position + 4 <= data.Length)
        {
            ...
            position += 4;
            if (position + size * 4L > data.Length) throw new InvalidDataException($"{acdPath}: entry {name} runs past the end of the file");
    ...
    public static string KeyFor(string folderName)
    {
        ...
        return string.Join("-", new[] { k1, k2, k3, k4, k5, k6, k7, k8 }.Select(k => (k & 0xFF).ToString()));
    }
}
